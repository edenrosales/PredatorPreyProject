...
//x and y positions on the World 
struct Position
{
    int x;
    int y;
...
private:
    ...
    //Makes all of the organisms of the type BombBug explode.
    //It will iterate through all the locations in world and find where there
    //is an instance of BombBug, since this function will destroy the organism,
    //we do not need to check if it has exploded or not yet, only if it is its
    //turn to explode
    //void explodeBombs(OrganismType);
};
